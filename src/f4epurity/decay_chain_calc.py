...
irrad_scenarios = {
    "DT1": {
        "times": [730.5 * day_to_sec, 730.5 * day_to_sec, 730.5 * day_to_sec, 730.5 * day_to_sec, 730.5 * day_to_sec, 600],
...
def irradiate(time, flux, parent_atoms, nuclide, decay_data_dic, lambda_temp, parent):
    ...
    for lambd, daughter in zip(decay_data_dic[nuclide]['lambda'][1:],decay_data_dic[nuclide]['Decay_daughter_names'][1:]):
        ...
        nuclide_out = irradiate(time,flux, parent_atoms, daughter, decay_data_dic, lambda_temp, parent)
...
def get_nuclides(scenario, parent, decay_data_dic, atoms):
    ...
    for time, flux in  zip(scenario["times"],scenario["fluxes"]):
        ...
        for nuclide in init_nuclides:
            ...
            # Irradiate this nuclide and work out the number of each nuclide in the decay chain
            out_nuclides = irradiate(time, flux, init_nuclides[nuclide], nuclide, decay_data_dic, lambda_temp, parent)
    ...
    for parent in nuclide_dict:
        ...
        for reaction in nuclide_dict[parent]["reactions"]:
            if isinstance(nuclide_dict[parent]["reactions"][reaction], list):
                nuclide_dict[parent]["reactions"][reaction] = np.array(nuclide_dict[parent]["reactions"][reaction])

        # Get the maximum length of the reactions lists
        max_len = max(len(values) for values in nuclide_dict[parent]["reactions"].values())
        ...
        for i in range(max_len):
            ...
            temp_nuclide_dict = {
                parent: {
                    ...
                    "reactions": {reaction: np.array([nuclide_dict[parent]["reactions"][reaction][i]]) for reaction in nuclide_dict[parent]["reactions"] if i < len(nuclide_dict[parent]["reactions"][reaction])}
                }
            }
            decay_data_dic = create_dictionary(decay_data, parent, temp_nuclide_dict[parent]["reactions"])
            final_nuclides = get_nuclides(irrad_scenario, parent, decay_data_dic, temp_nuclide_dict[parent]["atoms"])

            for nuclide in final_nuclides:
                # Only output unstable nuclides
                if "half_life_secs" in decay_data_dic[nuclide]:  
                    activity = final_nuclides[nuclide]*decay_data_dic[nuclide]["lambda"][0]
