...
def extract_dose_factors(nuclide, df):
    ...
    return nuclide_row.iloc[:, 1].values[0]  # assuming dose factor is in the second column
...
def dose_from_line_source(dose, x1, y1, z1, x2, y2, z2, x, y, z):  
    ...
    cos_angle = np.dot(vector_to_start, vector_to_end) / (np.linalg.norm(vector_to_start) * np.linalg.norm(vector_to_end))
    ...
    distance_perpendicular = np.linalg.norm(np.cross(source_end - source_start, point - source_start)) / np.linalg.norm(source_end - source_start)
    ...
    if distance_perpendicular == 0:
        ...
        dose_at_point = dose_per_length * angle_subtended / (distance_perpendicular * 4 * pi)
...
def is_within_bounds(x1, y1, z1, x2=None, y2=None, z2=None): 
    ...
    for stl_file in stl_files:
        ...
        if x2 is not None and y2 is not None and z2 is not None:
            ...
            if bounds[0] <= x1 <= bounds[1] and bounds[2] <= y1 <= bounds[3] and bounds[4] <= z1 <= bounds[5] and \
               bounds[0] <= x2 <= bounds[1] and bounds[2] <= y2 <= bounds[3] and bounds[4] <= z2 <= bounds[5]:
                bounds_list.extend(bounds)
                bounds_list.append(stl_file)
        else:
            # Check if the point lies within the bounds - point source case
            if bounds[0] <= x1 <= bounds[1] and bounds[2] <= y1 <= bounds[3] and bounds[4] <= z1 <= bounds[5]:
                bounds_list.extend(bounds)
                bounds_list.append(stl_file)
    ...
    if not bounds_list:
        ...
        print("WARNING: The point(s) of the source are not within the bounds of any of the available ITER room stl files.")
        return None
...
def write_vtk_file(dose, x1, y1, z1, run_dir, x2=None, y2=None, z2=None):
    ...
    if plot_bounds is None:
        if x2 is not None and y2 is not None and z2 is not None:  # Line source case
            ...
            plot_bounds = [x_center - 500, x_center + 500, y_center - 500, y_center + 500, z_center - 500, z_center + 500]
        else:  # Point source case
        ...
        for i in range(len(x)-1):
            for j in range(len(y)-1):
                for k in range(len(z)-1):
                    # Calculate the dose at the point from the line source
                    dose_point = dose_from_line_source(dose, x1, y1, z1, x2, y2, z2, x[i], y[j], z[k])
    ...
    # Create a 3D numpy array filled with the dose values based on 1/r^2
        ...
        for i in range(len(x)-1):
            for j in range(len(y)-1):
                for k in range(len(z)-1):
                    distance = np.sqrt((x[i] - x1)**2 + (y[j] - y1)**2 + (z[k] - z1)**2)
...
def plot_slice(dose_array, x, y, z, slice_axis, slice_location, plot_bounds):
    ...
    if slice_axis in axis_dict:
        slice_index = (np.abs([x, y, z][axis_dict[slice_axis]] - slice_location)).argmin()
    else:
        raise ValueError(f"Invalid slice axis: {slice_axis}. Expected one of: 'x', 'y', 'z'.")

    # Create a 2D mask for zero values
    mask = np.ma.masked_where(dose_array.take(slice_index, axis=axis_dict[slice_axis]) == 0, dose_array.take(slice_index, axis=axis_dict[slice_axis]))
    ...
    im = ax.contourf(coord1, coord2, transposed_mask, levels=levels_plot, cmap='rainbow', norm=LogNorm(), extent=[x.min(), x.max(), y.min(), y.max()])
    ...
    contours = ax.contour(coord1, coord2, transposed_mask, levels=levels_contour, colors='black')
    ...
    if len(plot_bounds) == 7:
        ...
        for solid in stl.solids:
            ...
            pairs = getattr(slice, axis_pairs[slice_axis][0] + '_pairs'), getattr(slice, axis_pairs[slice_axis][1] + '_pairs')
