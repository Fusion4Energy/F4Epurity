...
from f4epurity.maintenance import dose_within_workstation, get_dose_at_workstation, read_maintenance_locations
from f4epurity.reaction_rate import calculate_reaction_rate
from f4epurity.utilities import calculate_number_of_atoms, get_isotopes, sum_vtr_files, normalise_nuclide_name, get_reactions_from_file

def parse_arguments():
    # Define the command-line arguments for the tool 
    parser = argparse.ArgumentParser(description='Approximate the deviation in activity and dose rate based on local DR/NCR')
    parser.add_argument('--element', required=True, type=str, help ='Element with deviation e.g. Co, Ta, Nb')
    parser.add_argument('--delta_impurity', required=True, type=float, help ='Deviation in the elements weight as a percentage')
    parser.add_argument('--input_flux', required=True , help ='Provide a path to the vtr file with the neutron spectrum')
    parser.add_argument('--irrad_scenario', required=True, type=str, help ='Irradiation scenario. SA2 and DT1 are available or supply path to file with user defined scenario')

    parser.add_argument('--x1', nargs='+', required=True, type=float, help='x coordinate of point source')
    parser.add_argument('--y1', nargs='+', required=True, type=float, help='y coordinate of point source')
    parser.add_argument('--z1', nargs='+', required=True, type=float, help='z coordinate of point source')
    parser.add_argument('--x2', nargs='+', type=float, help='x coordinate of the second point for line source')
    parser.add_argument('--y2', nargs='+', type=float, help='y coordinate of the second point for line source')
    parser.add_argument('--z2', nargs='+', type=float, help='z coordinate of the second point for line source')

    parser.add_argument('--decay_time', required=True, type=float, help = 'Decay time for calculating dose in seconds')

    parser.add_argument('--plot', nargs=2, metavar=('slice_axis', 'slice_location'), help='Output image of dose mesh at a given location')

    parser.add_argument('--workstation', type=str, help="Name of the workstation for which to report the max dose e.g. 1, 3, 4 or 'all'")
    parser.add_argument('--location', type=str, help='Location of the workstation(s) e.g. Nb cell')
...
def calculate_dose_for_source(args, x1, y1, z1, run_dir, x2=None, y2=None, z2=None):
    ...
    for parent, product in reactions:
        ...
        if parent not in reaction_rates:
            ...
            number_of_atoms = calculate_number_of_atoms(parent, args.delta_impurity, nist_df)
        ...
        sigma_eff, flux_spectrum = collapse_flux(xs_values, args.input_flux, x1, y1, z1, x2, y2, z2)

        # Calculate the reaction rate based on the flux and effective cross section
        reaction_rate = calculate_reaction_rate(args.delta_impurity, sigma_eff, flux_spectrum)
    ...
    activities = calculate_total_activity(reaction_rates, args.irrad_scenario, args.decay_time, decay_data)

    # Initialize a list to store the total dose for each element
    total_dose = None

    dose_matrix_resource_path = pkg_resources.path('f4epurity.resources', 'F4E_dosematrix.xlsx')
    ...
    dose_array, x, y, z, plot_bounds = write_vtk_file(total_dose, x1, y1, z1, run_dir, x2, y2, z2)

    # Run quick plot function to output png image
    if args.plot:
        plt = plot_slice(dose_array, x, y, z, args.plot[0], int(args.plot[1]), plot_bounds)
...
def calculate_dose_at_workstations(args, dose, x1, y1, z1, run_dir, x2=None, y2=None, z2=None):
    ...
    if args.workstation:
        ...
        coordinates_list, workstation_list = read_maintenance_locations(args.workstation, args.location)

        # Create the output directory if it doesn't exist
        os.makedirs('output', exist_ok=True)

        # Create file with the dose values for each workstation
        filename = f'{run_dir}/dose_{x1}_{y1}_{z1}_{args.location}.csv' if x2 is None else f'{run_dir}/dose_{x1}_{y1}_{z1}_to_{x2}_{y2}_{z2}_{args.location}.csv'
        with open(filename, 'w', newline='') as f:
            ...
            for coordinates, workstation in zip(coordinates_list, workstation_list):
                ...
                max_dose, max_dose_coord = get_dose_at_workstation(dose, (x1, y1, z1), (x2, y2, z2), min_x, max_x, min_y, max_y, min_z, max_z, x2 is not None)
...
def process_sources(args):
    ...
    if args.x2 is not None and args.y2 is not None and args.z2 is not None:
        ...
        for x1, y1, z1, x2, y2, z2 in zip(args.x1, args.y1, args.z1, args.x2, args.y2, args.z2):
            dose_array, x, y, z, dose = calculate_dose_for_source(args, x1, y1, z1, run_dir, x2, y2, z2)
            dose_arrays.append(dose_array)
            calculate_dose_at_workstations(args, dose, x1, y1, z1, run_dir, x2, y2, z2, )
    else:
        # Point source
        ...
        for x1, y1, z1 in zip(args.x1, args.y1, args.z1):
            dose_array, x, y, z, dose = calculate_dose_for_source(args, x1, y1, z1, run_dir)
            dose_arrays.append(dose_array)
    ...
    if len(dose_arrays) > 1:
        ...
        if args.workstation:
            ...
            with open(f'{run_dir}/dose_{args.location}_total.csv', 'w', newline='') as f:
                writer = csv.writer(f)
                ...
                for coordinates, workstation in zip(coordinates_list, workstation_list):
                    ...
                    max_dose = dose_within_workstation(f'{run_dir}/dose_total.vtr', box_bounds)
