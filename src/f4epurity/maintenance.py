...
def read_maintenance_locations(workstation, location):
    ...
    if workstation.lower() == "all":
        ...
        df_filtered = df[
            (df["workstation"] == str(workstation)) & (df["location"] == str(location))
        ]
        ...
        coordinates = [
            df_filtered[["x_min", "x_max", "y_min", "y_max", "z_min", "z_max"]]
            .values[0]
            .tolist()
        ]
...
def get_dose_at_workstation(
    ...
    if is_line_source:
        ...
        if (
            (
                min_x <= source_end[0] <= max_x
                and min_y <= source_end[1] <= max_y
                and min_z <= source_end[2] <= max_z
            )
            or (
                source_start[0] <= min_x <= source_end[0]
                or source_start[0] >= max_x >= source_end[0]
            )
            and (
                source_start[1] <= min_y <= source_end[1]
                or source_start[1] >= max_y >= source_end[1]
            )
            and (
                source_start[2] <= min_z <= source_end[2]
        ...
        distance = np.sqrt(
            (closest_x - source_start[0]) ** 2
            + (closest_y - source_start[1]) ** 2
            + (closest_z - source_start[2]) ** 2
        )
