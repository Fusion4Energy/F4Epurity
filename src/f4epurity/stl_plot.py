...
class STLFacet:
    ...
    def coordinate_in_bounding_box(self, coordinate, tol=1e-6):
        if coordinate is None:
            ...
            return (self.x_min - tol <= coordinate[0] <= self.x_max + tol) \
...
class STLSolid:
    ...
    def slice(self, plane='x', intercept=0.0, create_loops=False):
        ...
        for i, facet in enumerate(self.facets, start=1):
            # line 1 - v1 v2
            inter_1 = vector_plane_intersection(facet.vertices[0], facet.vertices[1], vec_intercept, vec_projects)
            inter_1 = inter_1 if facet.coordinate_in_bounding_box(inter_1) else None
           
            # line 2 - v1 v3
            inter_2 = vector_plane_intersection(facet.vertices[0], facet.vertices[2], vec_intercept, vec_projects)
            inter_2 = inter_2 if facet.coordinate_in_bounding_box(inter_2) else None
 
            # line 3 - v2 v3
            inter_3 = vector_plane_intersection(facet.vertices[1], facet.vertices[2], vec_intercept, vec_projects)
            ...
            inters = [inter for inter in [inter_1, inter_2, inter_3] if inter is not None]
...
class STL:
    def __init__(self, file_path, format='ascii'):
        ...
        PARSE_METHODS = {'ascii': self._parse_stl_ascii,
    ...
    def _parse_stl_ascii(self, file_path):
        ...
        with open(file_path, 'r') as input_file:
            ...
            solid_indices_starts = [i for i, row in enumerate(rows) if row[:5] == 'solid']
            solid_indices_ends = [i for i, row in enumerate(rows) if row[:8] == 'endsolid']
            ...
            for i_start, i_end in zip(solid_indices_starts, solid_indices_ends):
                ...
                if len(section_solid[1:-1]) % 7 == 0:
                    sections_facets = [section_solid[1:-1][i:i+7] for i in range(0,len(section_solid[1:-1]),7)]
                else:
                ...
                for section_facet in sections_facets:
                    ...
                    normal = np.array([float(val) for val in section_facet[0].split()[-3:]])
                    vertex_1 = np.array([float(val) for val in section_facet[2][:-1].split()[-3:]])
                    vertex_2 = np.array([float(val) for val in section_facet[3][:-1].split()[-3:]])
                    vertex_3 = np.array([float(val) for val in section_facet[4][:-1].split()[-3:]])
