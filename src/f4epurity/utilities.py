...
def get_isotopes(element, nist_df):
    ...
    isotopes_df = nist_df[(nist_df['Atomic Symbol'].str.strip() == element.lower()) & (pd.notna(nist_df['Isotopic Composition']))]

    # Concatenate the 'Atomic Symbol' and 'Mass Number' columns
    isotopes = (isotopes_df['Atomic Symbol'].str.strip() + isotopes_df['Mass Number'].astype(str)).tolist()
...
def get_molar_mass(isotope, nist_df):
    ...
    df_filtered = nist_df[(nist_df['Atomic Symbol'] == atomic_symbol) & (nist_df['Mass Number'] == mass_number)]

    # If no matching row is found, return an error
    if df_filtered.empty:
        print(f"Error: No matching atomic symbol and mass number found for {atomic_symbol}, {mass_number}")
        sys.exit(1)
...
def sum_vtr_files(dose_arrays, x, y, z, run_dir):
    ...
    pyevtk.hl.gridToVTK(f'{run_dir}/dose_total', x, y, z, cellData={'Dose_Total': sum_dose})
...
def convert_names(nuc_dict):
    ...
    for parent in nuc_dict:
        ...
        for daughter in nuc_dict[parent]["reactions"]:
            new_dict[new_name]["reactions"][get_name(daughter)] = nuc_dict[parent]["reactions"][daughter]
