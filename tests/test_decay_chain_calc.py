...
def test_calculate_total_activity():
    ...
    nuclide_dict = {
        "co59": {"atoms": 1.02186E+25, 
                 "reactions":{
                    "co60m": [1.08E-14],
                    "co60": [3.05E-14],
                    }
                },
        "Nb093": {"atoms": 6.48194E+24,
                 "reactions":{
                    'Nb094m': [2.49E-15],
                    'Nb094': [1.16E-15],
                    'Nb092m': [4.02E-18],
    ...
    expected_output = {
        "Co060": np.array([2.53773E+08]),
        "Co060m": np.array([0.0]),
        "Nb094m": np.array([0.0]),
        "Nb094": np.array([5.05530E+03]),
        "Nb092m": np.array([2.14198E+04]),
        "Nb092": np.array([5.32794E-03]),
        "Nb093m": np.array([6.07715E+04]),
    }
...
def test_correction_factors():
    ...
    time_factors_path = os.path.join(os.path.dirname(__file__), 'data', 'd1s_time_correction_SA2')
    with open(time_factors_path, 'r') as f:
        time_correction_factors= {}

        for line in f:
            stripped_line = line.strip()
            if stripped_line.startswith('C') or stripped_line.startswith('nsc') or not stripped_line:
                continue
    ...
    for nuclide in time_correction_factors:
        nuclide_dict = {"co59": {"atoms": 1E+25, 
                "reactions":{
                nuclide: [1E-14]}
                }}
        activity = calculate_total_activity(nuclide_dict, irrad_sch, decay_time, decay_data)
    ...
    assert updated_activity[nuclide][0]/1e25/1e-14 == pytest.approx(time_correction_factors[nuclide], rel=0.05)
